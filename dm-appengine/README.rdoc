...
- Update does a get then a put. There's no reason to do the get if we can
  save the Entity in the resource whenever we get it from the Datastore.
  Doing this probably requires a custom Resource type, like the CouchDB adapter.
